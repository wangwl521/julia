...
(define (compile-body e vi lam)
  (let ((code '())            ;; statements (emitted in reverse order)
        ...
        (try-label-stack '())) ;; label stack for entry points of current try blocks
    (define (emit c)
    ...
    (define (emit-return x)
      (define (actually-return x)
        (let* ((x   (if rett
          ...
          (if tmp (emit `(= ,tmp ,x)))
          (if (pair? try-label-stack)
             (emit `(pop_exc ,(last try-label-stack))))
    ...
    (define (compile e break-labels value tail (linearize-args #t))
      (if (or (not (pair? e)) (memq (car e) '(null ssavalue quote inert top core copyast the_exception $
          ...
          (case (car e)
            ...
            ;; (pop_exc LE) - pop exception stack back to state of enter at label LE
            ((trycatch tryfinally)
             (let ((enterl (make-label))
                   (catch (make-label))
                   (endl  (make-label))
               ...
               (mark-label enterl)
               (emit `(enter ,catch))
               (set! try-label-stack (cons enterl try-label-stack))
               ...
               (let* ((v1  (compile (cadr e) break-labels value #f))
                 ...
                 (if finally
                     ...
                     (let ((v2 (compile (caddr e) break-labels value tail)))
                       (if (not tail) (emit `(pop_exc ,enterl)))
                       (if val (emit-assignment val v2))))
                            ...
                            (let loop ((actions (caddr my-finally-handler)))
                              (if (pair? actions)
                                  (let ((skip (if (and tail (null? (cdr actions))
                                    ...
                                    (loop (cdr actions)))))))
                 (set! try-label-stack (cdr try-label-stack))
                 val)))
